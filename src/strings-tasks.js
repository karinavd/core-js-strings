...
function removeFirstOccurrences(str, value) {
  const indexStr = str.indexOf(value);
  return indexStr === -1
    ? str
    : str.slice(0, indexStr) + str.slice(indexStr + value.length);
}
...
function removeLastOccurrences(str, value) {
  const indexStr = str.lastIndexOf(value);
  return indexStr === -1
    ? str
    : str.slice(0, indexStr) + str.slice(indexStr + value.length);
}
...
function sumOfCodes(str) {
  return isString(str)
    ? str.split('').reduce((sum, x) => sum + x.charCodeAt(0), 0)
    : 0;
}
...
function countVowels(str) {
  const numberBool = str.match(/[aeioyuAEIOUY]/gi);
  return numberBool ? numberBool.length : 0;
}
...
function isPalindrome(str) {
  const string = str.toLowerCase().replace(/[^0-9A-Za-z]/g, '');
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    const letter = char <= 'Z' ? 65 : 97;
    return String.fromCharCode(
      ((char.charCodeAt(0) - letter + 13) % 26) + letter
    );
  });
}
